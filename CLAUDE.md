...
**Rewrite rules:** Data-driven simplification via `.fw` patterns. 26 builtin rules (trig symmetry, inverse pairs, abs, log/exp, power rules, division / reciprocal cancellation, complex identity `i^2=-1`, negative-exponent normalization `x^n = 1/x^(-n) iff is_neg_num(n)`). Commutative flattened matching handles N-term additive/multiplicative permutations. Rules loaded from `BUILTIN_REWRITE_RULES` string; user `.fw` files can add more. `RewriteRule::condition` is `std::optional<Condition>`, parsed once at rule-load time by `parse_condition` (system.h) and evaluated per match by `check_condition` (expr.h free function). `||` in rule conditions works correctly. `Condition`/`CondClause`/`CondOp`/`CondLogic` structs and `check_condition` live in `expr.h` (after `ValueSet`, before `RewriteRule`). **Typed-binding predicates** (since 2026-05-10, Future #53): rule conditions accept typed predicates that test the runtime binding of a wildcard. Current predicate set (3 shipped): `is_neg_num(n)` (negative numeric literal), `is_in_dimension(v, dim)` (2-arg; checks `dim_map_` entry for the Var that `v` binds to), `is_int(n)` (integer-valued Num). All ship in `PREDICATE_NAMES` set in `expr.h` and are recognized by `is_predicate_clause`. Fail-safe semantics throughout: unknown or non-matching binding → false. `is_neg_num` and `is_int` are 1-arg predicates; `is_in_dimension` is 2-arg (lhs=Var wildcard, rhs=literal dim-atom Var). Encoded as `CondClause{lhs=FUNC_CALL(...), rhs=nullptr, op=CondOp::EQ}` — FUNC_CALL-in-lhs with sentinel null rhs, NOT a new clause type. `check_condition` has an optional 3rd param `const map<string, ExprPtr>* expr_bindings = nullptr` and a 4th optional `const map<string, string>* dim_map = nullptr` (added gen-3 cycle 2, 2026-05-15); the 14 system.h comparison-clause call sites are unchanged (both params defaulted). `RewriteRulesGuard` ctor extended 4→5 args (5th is `dim_map` ptr, defaulted); passes `&dim_map_` from the three construction sites in system.h. Predicate set extends per-consumer schedule (Future #65); remaining: `is_pos_num`, `is_num`.
...
**Symbolic provenance carrier:** `solved_symbolic_` (`mutable map<string, ExprPtr>`) and `aliases_` (`mutable map<string, double>`) on `FormulaSystem` carry symbolic forms and the alias-resolution table alongside the numeric `bindings` map, so `--steps`/`--calc` trace sites render from the same ExprPtr as final output — trace and final cannot disagree. `fmt_trace(double, ExprPtr=nullptr, key="")` is the single unified render helper. `dim_map_` (`map<string, string>` variable→dim-atom; shipped gen-3 cycle 2, 2026-05-15) is the third parallel map — currently `using DimName = std::string`; promotes to `map<DimName, int>` exponent algebra in cycle 3. Cross-file propagation: `load_sub_system` copies `dim_map_` into the child system. Future #82 (PARKED): consolidate the parallel-map family when a 4th appears.
...
### Dimension annotations (since gen-3 cycle 2, 2026-05-15)
```
[mass]                          # bare [name] section declares a dimension category
g = 1                           # bindings inside become dim-typed variables
kg = 1000 * g

m_obj:mass = 10 * kg            # : annotates a binding with its dimension atom
n:(int, mass) = 5               # intersection: multi-type annotation; atom-list only
mass.kg                         # dot-dispatch resolves into the [mass] dim section
```
- `[name]` with no parens and no `->` is a dimension section (`is_dimension_section` predicate). Its body registers all LHS names (from both `equations` and `defaults`) into `dim_map_`.
...
- `is_in_dimension(v, dim)` predicate in rule conditions checks `dim_map_[v_binding_name] == dim`. Enables stdlib `.fw` dimensional rejection rules.
- `dim_map_` propagates into child systems via `load_sub_system`. `DimName = std::string` (cycle 3 will promote to `map<DimName, int>` exponent algebra).
