...
**Before spawning review agents**: run `make analyze` yourself as a single background task (~45 min); do NOT let review agents run it. Check duplicate-launch via either (a) the sentinel file `[ -f /tmp/fwiz-analyze.running ]` (write on launch, remove on exit — most reliable), or (b) `ps -ef | grep -E 'clang-tidy|cppcheck' | grep -v grep | grep -v zsh | grep -v bash` (filters wrapper shells). Do NOT use `pgrep -f <token>` of any kind — even with a "unique tag" the token is in the orchestrator's own argv (see Background Task Discipline rule #4). Spawn order (staggered parallelism saves ~20-30 min wall-clock): IMMEDIATELY `doc-updater` + `perf-auditor` in parallel (neither needs analyze); AFTER analyze returns, `reviewer` (needs it to audit Collected Issues). If analyze already complete on arrival (multi-milestone cycle run mid-cycle), spawn all three in parallel.
...
3. **Hung-task threshold**: 2x expected duration for *silent* hangs (no output). `make analyze` takes ~45 min; not hung before 90 min silence. **Oracle-contention escape**: if `make analyze` etime exceeds 4x expected (~3h) AND `ps -o %cpu,comm ax | sort -rn -k1 | head -5` shows a non-fwiz process consuming > 200% CPU (system contention, not analyze itself), surface to the user: "analyze running 4x expected under {process} contention — defer to batch run? (yes/no)". Do not silently wait through extreme contention; user has authority to invoke the oracle-less protocol but cannot if not asked. Canonical miss: T1 cleanup cycle 2026-04-28 — clang-tidy ran 18.5h (25x expected) under llama-server at 546% CPU before the user proactively offered batch-defer. The 4x trigger would have surfaced the decision at ~3h.
