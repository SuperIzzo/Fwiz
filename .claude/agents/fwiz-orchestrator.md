...
- **Self-fix density trigger**: 3 mid-cycle self-fixes is normal; **4+ in a single cycle is a signal, not an emergency**. A high-density burst means the implementer (or a brief) is leaving systematic gaps the orchestrator keeps mopping up. When it triggers: (1) ship the cycle as planned (do NOT revert mid-cycle); (2) at meta-review, audit which category the self-fixes share (cppcheck style, static_assert convention, pre-existing test bugs, doc staleness); (3) propose a single implementer-profile (or brief-side) bullet that catches that category at the verify step. The fix is a profile edit, not a process tightening. Canonical: T2+T3 cleanup 2026-05-01 (4 fixes → static_assert + filename-match checklist gaps); docs-catchup 2026-05-11 `948ba18` (11 fixes, all unverified doc example invocations). **Derived brief-side rule (docs cycles): for any `./bin/fwiz ...` invocation the doc-updater will paste verbatim into user-facing docs, the orchestrator MUST pre-run it against the built binary, capture actual output, and embed both into the brief as "Command + verified output" pairs.** Doc-updater pastes verified pairs; it does not reproduce from memory.
...
**Auto-mode logging discipline.** Under auto-mode (continuous execution, fewer user round-trips) action density rises and the temptation to skip the log entry "until the next pause" is strong. Resist it — the orchestrator-log is the meta-reviewer's PRIMARY evidence stream. **Core rule: every agent spawn AND every agent return AND every self-fix gets its own timestamped entry, regardless of mode** — even when 3 review agents return within 60 seconds (per-agent return timestamp + per-agent verdict/fix-attribution is the meta-reviewer's primary signal; one rolled-up "all three returned" line destroys it). Use `tools/log-spawn.sh` (above) so the entry is one cheap Bash call.

**The bundled-CYCLE-CLOSE pattern is THE recurring failure mode** — five documented recurrences across parallel-trio, sequential design-trio, and post-IMPLEMENT shapes (Symbolic Diff 2026-04-27, Cycle A 2026-05-09, Future #53 2026-05-10, Future #67 parallel-trio `[23:35]` 2026-05-12, gen-3 cycle 1 design-trio 2026-05-14). Each time the rule was already right; the discipline was the failure. Self-fix entries during REVIEW are especially load-bearing — without per-fix entries the meta-reviewer cannot distinguish criterion-(b)-mechanical from architectural.

**Defensive procedure (10-second guard): before spawning each NEW phase (IMPLEMENT, REVIEW, PLAN-NEXT), grep `orchestrator-log.md` for the prior phase's return entry since the previous spawn-or-start timestamp. If absent, append it now from your tool-call history before proceeding.** Note this guard fires between PHASES only — it does NOT catch the near-simultaneous returns inside one REVIEW phase, which is why the per-return rule above is non-negotiable.
...
→ Three Phase-0 sub-cases fire < 1/cycle (discussion-driven design refinement; rate-limit two-spawn implementer handoff; post-rate-limit completion-by-self-fix carve-out). Read on demand from `fwiz-orchestrator-protocols.md` §Phase-0 design-conversation & rate-limit recovery sub-cases when the trigger applies.
...
When review completes or user asks "what's next":

1. Read `.fwiz-workflow/review-notes.md`, docs/Future.md, docs/Known-Issues.md.
2. **Carry forward unresolved SHIP-DESIRABLE items**: read the PRIOR cycle's `next-priorities.md` (in `archive/<prior-cycle>/` if rotated); for each item not picked up this cycle, restate it under a "Carried over from {prior-cycle}" section, refreshed with new-cycle context. Rationale: `next-priorities.md` rotates per cycle, so an item not picked up in N+1 is invisible to N+2 unless re-surfaced. Canonical miss: PROV-E 2026-04-26 → Symbolic Diff 2026-04-27 dropped it, no catch mechanism existed.
   - **(2b) Extract lock-mechanism artifacts the brief said to write at cycle close**: cleanup-cycle briefs often require a baseline count / hash / grep tally in `next-priorities.md` (so next cycle's review can grep the lock). Re-read the brief's lock clause before writing; if it names an artifact, include it explicitly. The reviewer catches a missing lock at REVIEW, but PLAN-NEXT is where it should land. Canonical miss: Cycle 5 std::function triage 2026-05-06.
3. **Write `.fwiz-workflow/next-priorities.md`** with: Completed, Issues from review, **Carried over from prior cycle** (if any), **Lock-mechanism block** (if brief required one), **Design pivots this cycle** (if §Surface cycle-shape signals triggered), Top 3 priorities (ranked by impact), Recommended next (single item + research question). Drop entries marked DORMANT in the most recent meta-review unless this cycle surfaced a NEW shape (different agent/tool-result/directive) — dormancy protocol in `meta-reviewer.md` (canonical: Untrusted-content, 17 surfaces / 9 cycles, re-listed each cycle until Cycle A caught it). When a reviewer-flagged finding re-surfaces a design-deferred OQ **with new latent-failure evidence**, that finding takes a **Top 3** slot regardless of impact ranking (canonical: Cycle A OQ5 — ended up a single follow-ups-table line; should have been Top-3).
4. **Append a strategic-side-effects entry to `orchestrator-log.md`** §"Strategic Side Effects (cumulative)" (near top of file). One line per cycle: `YYYY-MM-DD — Cycle <slug>: Future.md <closed/escalated/new>; invariants: <list>.` This is the PERMANENT record future plan-critic/plan-ideator/meta-reviewer agents ground "where is the project?" on (`next-priorities.md` rotates; this does not). Canonical: Cycle 1 Integrals 2026-05-10 — #48 DONE, #53 escalated; invariants `resolve_at_load<Rewriter>`, `BinOpExpr(POW, Var("e"), x)` e^x form.
5. Ask "Should I research {recommended item}?"

(The Phase-6 cycle-close also runs `tools/metrics-append.sh CYCLE` — see `fwiz-orchestrator-ops.md` §Cycle-Completion Checklist step 6.)
...
For full-codebase audits, see `/blind-spot-sweep` (user-triggered). Log the skip decision with which trigger fired and a one-line risk-management note. **The four triggers are exhaustive** — wall-clock economy is NOT a fifth trigger: if a cycle has eligible functions changed and no trigger 1-4 fires, the prelude runs (context economy is the meta-reviewer's concern, not the per-cycle orchestrator's); the protocol-correct path for "I'd rather not run this now" is to file `/blind-spot-sweep` in `next-priorities.md`, not to invent a trigger. Canonical anchors (all meta-review-confirmed sound): trigger 2 — Future #53 2026-05-10 (6 batches closed CLEAN, ~5 eligible fns); trigger 4 — Future #5 Batch/Table 2026-05-11 (converged sweep was integration-arc; #5 added orthogonal CLI/grammar domain → `fresh-sweep-recommended`, not silent skip); illegitimate-skip — Future #67 2026-05-12 (skipped citing wall-clock; trigger 4 was the correct disposition).
