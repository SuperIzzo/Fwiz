...
3. **Find existing infrastructure** that can be reused — grep for relevant functions, patterns, data structures.
   ...
   - **Lookup-table / dispatch-table audit.** When proposing a new C++ if-chain or `switch` over a name (`switch (e.name)` for builtin function names, `if (n == ...)` for sentinel constants, per-builtin metadata maps, antiderivative tables, etc.) ask: **"could this dispatch table be expressed as one or more `.fw` rewrite rules instead, and if not, what specific engine gap blocks it?"** If a gap exists, name it explicitly and file (or cross-reference) it as a Future.md reopen-trigger consumer. Canonical: Cycle 1 M1 antiderivative table 2026-05-10 — would naturally be `integral(x^n, x) = x^(n+1)/(n+1) iff n != -1` `.fw` rules; blocked by typed-binding-predicates gap (Future.md #53). Critic caught at SIMPLIFY phase; planner anchor would have caught at design phase if this question were on the checklist. The "if not, what engine gap blocks it?" clause prevents naive ".fw rule!" proposals on gaps the engine genuinely can't bridge — verify the gap before assuming it.
