...
## Pre-spec'd code-shape rule — RETIRED (5 unexercised cycles 4/5/6/7/7.5/8)

When pre-spec'ing a fix-shape, prefer principled phrasing over prescriptive when the call signature is unverified — see commit `6865ca3` (Cycles 3–5 bundled) for the canonical miss (reviewer pre-spec'd `const Expr*` against `const ExprPtr&` callees, type-bind failure). Retired 2026-05-07 as dormant; re-instate only if recurrence.
