...
10. Run: `make test && make sanitize && make analyze-fast`. The first two are standard correctness gates; `make analyze-fast` (cppcheck, ~1-2 min) is your pre-handoff style/convention gate. Grep the cppcheck output for `warning:|error:|style:|performance:` and fix any non-zero counts before declaring DONE. **Foreground only — never background.** This carves out `analyze-fast` from the "never launch analyze" rule below; `analyze` (combined) and `analyze-full` (clang-tidy) remain orchestrator-owned. Empirical justification: across 4 consecutive cycles since the rule was added, cppcheck-style findings caught at orchestrator REVIEW have been **1+5+8 = 14 findings let-through pre-rule (3 cycles, Integrals M1/M2/M3); 0 findings let-through post-rule (1 cycle, cleanup-bundle 2026-05-10). Validated steady-state.** Mechanism: implementer's edits become cppcheck-clean *by construction* because the gate is internalized at GREEN. Most pre-rule findings were const-pointer cleanups, cstyleCast, dead-condition, and internalAstError false-positives.
...
- **Scratch-file hygiene**: Reproducer / probe / experiment files outside `src/` (e.g., `m4_repro.cpp`, `test_foo.fw` at repo root, `/tmp/repro*.cpp`) MUST be deleted before declaring DONE. Final report MUST contain a "Scratch artifacts" line listing every transient path created AND **`ls`-verifying each path is gone** — not "I ran rm". The verification is a separate Bash call: `ls /tmp/repro.cpp /tmp/repro2.cpp /tmp/test_dbg 2>&1` should return `No such file or directory` for each; paste that output into the Scratch artifacts line. Claim-without-verification has caused two recurrences: M4 cycle 2026-04-27 cleaned but did not report (clangd kept stale entries), and gen-3 cycle 2 2026-05-15 reported "all rm'd" in the implementation log but `/tmp/repro*.cpp` remained on disk and surfaced later as phantom compile-error diagnostics. The rule extension: claim + `ls` proof, not just claim. Stale scratch files trigger phantom IDE/clangd diagnostics that the orchestrator then has to chase.
