...
7. **Prior-art-user vs Fwiz-user alignment** (when external research finds "universal" or "unanimous" approaches across CAS / compilers / other tools). Before recommending an approach because "SymPy/Maple/Maxima/Mathematica all do X", identify whose **goal** the prior-art is optimizing for. Compiler CSE optimizes for codegen runtime — extracting every frequency-≥-N atom is correct because each saves a register/instruction. Fwiz's `--derive` user is a human reading equations or an LLM ingesting them — atoms-as-helpers (`t1=2*b`) HURT readability. If prior art's user differs from Fwiz's user, "universal across CAS" is evidence about the algorithm's correctness in its native domain, not about its fitness for Fwiz's defaults. Surface this gap explicitly in the brief: a one-line "Prior-art alignment: {tool}'s {feature} is for {user-X}; Fwiz's {feature} is for {user-Y} — defaults may need to differ" sub-section. Canonical miss: Cycle B `--cse` 2026-04-25 — research-external confirmed "frequency≥2 is universal CSE criterion across SymPy/Maple/Maxima/Mathematica"; design adopted frequency-threshold semantics; implementer measured 165 helpers (faithful to prior art); user's goal was human readability not codegen. Defaults had to be reframed mid-cycle (Option C: top-N by value). The prior-art-user gap was the load-bearing miss.

8. **Multi-throw-site probe rule** (solver-strategy or exception-routing research where the failure chain may exit at multiple throw sites). When the empirical bisection identifies a surface failure (e.g., "X throws at line N"), MUST probe what happens NEXT if that surface failure is patched or instrumented to skip. If a second throw site fires at a deeper depth, both must be enumerated in the brief — the design phase will otherwise target only the visible layer. Canonical miss: cycle 3j 2026-06-06 — probe identified depth re-throw at `system.h:4457`; design proposed depth=0-swallow + typed exception. Implementer discovered mid-GREEN that the visited-set Circular guard at depth ~2 fires BEFORE depth exhaustion under Strategy 2 EXPR's `eval_binding` path, intercepting the chain entirely. A second probe with the depth-rethrow patched would have surfaced the Circular interception immediately. Single-trace bisection mis-directs the design when failure chains have multiple exit points.
