...
## 13. Symbolic integration — current scope (M1+M2+M3, 2026-05-10)

Three-cycle Integrals arc complete. M1: indefinite Tier 1; M2: u-substitution, definite integrals, adaptive Simpson; M3: integration by parts via LIATE + `BuiltinMeta` registry. What works and what does not:

**Works:** ~25 atomic Tier 1 patterns: constants (`integral(c, x) → c*x`), power rule (`x^n → x^(n+1)/(n+1)`), `1/x → log(x)`, `e^x → e^x`, `sin/cos/tan(x)` antiderivatives, linearity over ADD/SUB, scalar MUL/DIV. M2 u-substitution via `try_u_sub_integrate` (e.g. `integral(2*x*cos(x^2), x) → sin(x^2)`, `integral(x*e^(x^2), x) → e^(x^2)/2`). Definite 4-arg form `integral(f, x, a, b)` with symbolic F(b)-F(a) primary path (e.g. `integral(x^2, x, 0, 3) → 9`). Adaptive Simpson numeric fallback for non-elementary definite integrands (e.g. `integral(e^(-x^2), x, 0, 1) ≈ 0.7468`). M3 integration by parts via LIATE heuristic (depth ≤ 3) — e.g. `integral(x*e^x, x) → x*e^x - e^x`, `integral(x^2*log(x), x) → x^3*log(x)/3 - x^3/9`, `integral(atan(x), x) → x*atan(x) - log(x^2+1)/2`. Nine builtins covered by `BuiltinMeta` registry (sin/cos/tan carry antiderivative entries; log/asin/acos/atan/sqrt/abs fall through to IBP or unevaluated). Two CLI surfaces: `integral(target, var)=?[alias]` (indefinite) and `integral(target, var, lo, hi)=?[alias]` (definite). Unrecognized forms preserve the unevaluated `integral(...)` FUNC_CALL — same convention as `diff(...)`; observable in `--steps` traces and output round-trip.

**Does not work yet:**
- Cyclic IBP (`e^x*sin(x)` family) — depth limit catches the recursion; result stays unevaluated by design. Reopen trigger: user reports family unevaluated in real reproducer AND cleanly-layered detection mechanism identified.
- IBP depth > 3 chains — depth limit is a deliberate bounds guard, not an algorithm limit; lifting it requires cyclic-IBP detection first.
- `+ C` constant of integration — deliberately excluded (would not round-trip as `.fw`).
- Risch algorithm, improper integrals, multi-variable integration, trig substitution, partial fractions, special functions — see cross-arc reopen triggers in Future #16.
