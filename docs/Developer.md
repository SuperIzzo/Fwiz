...
- Data-driven rewrite rules (`BUILTIN_REWRITE_RULES` string, 26 rules): applied after the structural rules above via `apply_rewrite_rules`. The two Tier 1 additions (G1: `k * x / (k * y) = x / y iff k != 0` and G3: `x / (1 / y) = x * y iff y != 0`) close numeric-common-factor cancellation and unit-fraction-denominator rewriting respectively. Rules with `iff cond` evaluate `check_condition` against numeric bindings built from the current pattern match; comparison clauses with unbound wildcards return `nullopt` (treated as satisfied — permissive for symbolic unknowns). This is intentional: `sqrt(x)^2 = x iff x >= 0` simplifies `sqrt(a)^2` to `a` even when `a` has no known sign, because the alternative — leaving the `sqrt^2` wrapper intact — adds no information and blocks downstream fingerprint dedup. The semantic cost is accepted: the condition annotation documents the domain constraint without enforcing it for symbolic unknowns. Rules that must NOT fire for unknown-sign variables (e.g. `abs(x) = x iff x >= 0`) require a principled domain-propagation mechanism before they can be added; see Future #31.
  ...
  **Typed-binding predicates** (since 2026-05-10, Future #53): rule conditions additionally support typed predicates such as `is_neg_num(n)`, which test the runtime binding of a wildcard rather than comparing its numeric value. `is_neg_num(n)` is true only when `n` binds to a negative numeric literal (`Num(v)` with `v < 0`) — fail-safe semantics: unknown binding or non-Num binding → false (not permissive-unknown). This enables rules like `x^n = 1 / x^(-n) iff is_neg_num(n)` to fire exclusively on negative-literal exponents without looping on symbolic ones. Encoding: a predicate clause is a `CondClause{lhs=FUNC_CALL("is_neg_num", {Var("n")}), rhs=nullptr, op=CondOp::EQ}` — FUNC_CALL-in-lhs with a null rhs sentinel, NOT a new clause type or new `CondOp` enumerator. Tooling walking `Condition.clauses` should detect predicates via `is_predicate_clause(c)` (checks `c.lhs->type == FUNC_CALL && PREDICATE_NAMES.count(c.lhs->name)`), not by checking `rhs == nullptr` alone. `check_condition` accepts an optional third param `const map<string, ExprPtr>* expr_bindings = nullptr`; when null (the 14 system.h comparison-clause sites), predicates resolve fail-safe to false. The predicate name set is `predicate_names()` (`std::unordered_set<string>`) in `expr.h`; extend it when adding a new predicate with a named consumer.
...
- **Sibling exceptions** — errors that must reach the user and must NOT be swallowed by solver-internal `catch (const std::runtime_error&)` sites derive directly from `std::exception` (NOT from `std::runtime_error`). Current sibling exceptions in `system.h`: `SolveBudgetExceededError`, `CrossFileResolutionCycleError`, `RaggedMatrixError`. The top-level catch in `main.cpp` and the test harness `get_error` both catch `const std::exception&` and see them. Convention: "errors that must reach the user are sibling exceptions; errors that are per-call recoverable derive from `std::runtime_error`."
