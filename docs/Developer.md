...
**Collections-as-call-arguments (gen-6 Collections Cycle 3, 2026-06-24):** named-section calls that carry a `seq`-shaped binding (e.g. `sum_of({1,2,3})`) are resolved by parent-side body inlining rather than through the numeric `prepare_sub_bindings` channel (which would drop the collection). Two splice sites in system.h: `try_formula` lambda in `solve_recursive` and the FORMULA_FWD branch in `solve_all`, both gated by `call_has_seq_arg(const FormulaCall&)` (~7-line `std::any_of` predicate). When the gate fires: `unfold_formula_call_body` inlines the section body with the seq substituted as an ExprPtr; `resolve_foldl_calls` folds the resulting `foldl(seq(...), op, init)`; `inline_seq_section_calls` cleans up surviving nested seq-bearing calls. `unfold_formula_call_body` was extended to re-inline nested formula calls stored as `_fcN` placeholders (needed for composed bodies like `mean_of`'s `sum_of(xs)/count_of(xs)`). A non-inlinable seq-arg (section body lacks a substitutable output equation) throws `FoldOperatorError` — never silent. **Cross-cycle invariant:** `prepare_sub_bindings` remains numeric-only by contract; all collection-valued section arguments resolve exclusively via `unfold_formula_call_body` on the forward-resolve path.
