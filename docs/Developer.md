...
- **Units / dimensional analysis** — hybrid dim model substrate shipped (gen-3 cycle 2, 2026-05-15): bare `[name]` dim sections, `:` annotation, intersection `(t1, t2, ...)`, `dim_map_` on `FormulaSystem`, `is_in_dimension` + `is_int` predicates, `BindingAnnotationError` sibling exception. See Future #78 (DONE-by-design), #7b BASIC (DONE), #65 (partial). Cycle 3 (conditional, demand-pull) adds `compute_dim` propagation algebra for compound expressions.
...
- Data-driven rewrite rules (`BUILTIN_REWRITE_RULES` string, 26 rules): applied after the structural rules above via `apply_rewrite_rules`. The two Tier 1 additions (G1: `k * x / (k * y) = x / y iff k != 0` and G3: `x / (1 / y) = x * y iff y != 0`) close numeric-common-factor cancellation and unit-fraction-denominator rewriting respectively. Rules with `iff cond` evaluate `check_condition` against numeric bindings built from the current pattern match; comparison clauses with unbound wildcards return `nullopt` (treated as satisfied — permissive for symbolic unknowns). This is intentional: `sqrt(x)^2 = x iff x >= 0` simplifies `sqrt(a)^2` to `a` even when `a` has no known sign, because the alternative — leaving the `sqrt^2` wrapper intact — adds no information and blocks downstream fingerprint dedup. The semantic cost is accepted: the condition annotation documents the domain constraint without enforcing it for symbolic unknowns. Rules that must NOT fire for unknown-sign variables (e.g. `abs(x) = x iff x >= 0`) require a principled domain-propagation mechanism before they can be added; see Future #31.
  ...
  **Typed-binding predicates** (since 2026-05-10, Future #53; extended gen-3 cycle 2 2026-05-15): rule conditions additionally support typed predicates that test the runtime binding of a wildcard. Three predicates ship: `is_neg_num(n)` (negative Num), `is_int(n)` (integer-valued Num), and `is_in_dimension(v, dim)` (2-arg: Var that `v` binds to has dim entry in `dim_map_`). All are fail-safe: unknown or non-matching binding → false (not permissive-unknown). Encoding: a predicate clause is `CondClause{lhs=FUNC_CALL(name, args), rhs=nullptr, op=CondOp::EQ}` — FUNC_CALL-in-lhs with a null rhs sentinel, NOT a new clause type or new `CondOp` enumerator. Tooling walking `Condition.clauses` should detect predicates via `is_predicate_clause(c)` (checks `c.lhs->type == FUNC_CALL && PREDICATE_NAMES.count(c.lhs->name)`), not by checking `rhs == nullptr` alone. `check_condition` has an optional 3rd param `const map<string, ExprPtr>* expr_bindings = nullptr` and a 4th optional `const map<string, string>* dim_map = nullptr`; the 14 system.h comparison-clause call sites pass neither (both defaulted). `is_in_dimension` is threaded via a thread-local pointer `simplify_dim_map_()` (mirrors `simplify_bindings_`) set by `RewriteRulesGuard` (RAII, 5-arg ctor — 5th is `dim_map` ptr). The predicate name set is `predicate_names()` in `expr.h`; extend it when adding a new predicate with a named consumer.
