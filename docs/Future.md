...
## #R9. Refactor: `main` function split (520 LOC, src/main.cpp:16-535)

**From:** Cycle blind-spot 2026-05-10 (function-scope, F1). Initial T1 mechanics score: H=match/specific, **G=match/vague (truncated at `--max-tokens 2048` mid-control-flow)**. Truncation = size-signal per the comprehension-gate principle: too many CLI-flag locals + lambdas + dispatch arms to fit in a single role-by-role mechanics pass.

**Diagnosis (T1-only, no intervention loop run):** size. Function holds 50+ named locals plus lambdas (`print_if_new`, `print_fit`, `is_exact_result`) plus 5 dispatch modes (explore, derive, fit, verify, plus diff/integral synthesis blocks).

**Proposed cut points** (each becomes a free function or static method):
- CLI flag-parsing block (lines 26-130) → `parse_cli_flags(argc, argv) → CLIFlags`
- explore-mode block → `run_explore_mode(sys, query, level)`
- --derive block → `run_derive_mode(sys, query, derive_cap, cse_threshold)`
- --fit block → `run_fit_mode(sys, query, fit_depth, output_file)`
- verify block → `run_verify_mode(sys, to_verify)`
- diff-query synthesis → `synthesise_diff_queries(query)`
- integral-query synthesis → `synthesise_integral_queries(query)`

`main` shrinks to ~50 LOC of dispatch + outer try/catch.

**Reopen trigger:** post-split, blind-spot re-runs `main` → mechanics passes Gemma without truncation.

## #R10. Refactor: `derive_all` function split (366 LOC, src/system.h:1434-1799)

**From:** Cycle blind-spot 2026-05-10 (function-scope, F2). Initial T1 mechanics score: H=match/specific, **G=match/vague (truncated)**. Size-signal: fingerprint+winners+CSE pipeline + fallback condition extraction in one body.

**Proposed cut points**:
- Test-points setup (lines ~1450-1480) → `prepare_test_points(free_vars) → vector<map<string,double>>`
- `consider_result` lambda → promote to private method `consider_candidate(probe, score, expr, winners, empty_fp_counter)`
- CSE post-processing block → `apply_cse_to_winners(winners, cse_threshold, occupied) → (helpers, for_format)`
- Fallback bidirectional extraction → `derive_from_conditions(target, equations, bindings) → vector<string>`

`derive_all` shrinks to ~80 LOC of orchestration.

**Reopen trigger:** post-split, blind-spot re-runs `derive_all` → mechanics passes Gemma without truncation.

## #R11. Refactor: `parse_cli_query` function split (268 LOC, src/system.h:3770-4037)

**From:** Cycle blind-spot 2026-05-10 (function-scope, F3). Initial T1 mechanics score: H=match/specific, **G=match/vague (truncated)**. Size-signal: query-syntax parsing across multiple sub-formats (filename, section, diff, integral, bindings) in one body.

**Proposed cut points**:
- Filename + section parsing (lines ~3795-3850) → `parse_filename_and_section(input, q)`
- Per-argument processing loop body → `parse_arg(arg, q, allow_no_queries, allow_symbolic)` — single-arg handler called from the loop
- diff-query branch → `parse_diff_query(name, val, q)` (already mostly self-contained)
- integral-query branch → `parse_integral_query(name, val, q)`
- Bindings parsing (numeric + expression fallback) → `parse_binding_value(name, val, q, allow_symbolic)`

`parse_cli_query` shrinks to ~50 LOC: split into args, dispatch each arg, validate.

**Reopen trigger:** post-split, blind-spot re-runs `parse_cli_query` → mechanics passes Gemma without truncation.

## #R12. Refactor: `match_pattern` decomposition (242 LOC, src/expr.h:811-1052)

**From:** Cycle blind-spot 2026-05-10 (function-scope, F4). Initial T1 mechanics score: H=match/specific, **G=match/vague (truncated)**. Size-signal: PatternMatcher functor with backtracking + commutative permutation + factor matching all in one body.

**Proposed move**: the nested `PatternMatcher` struct already separates concerns. Promote it to a file-scope `class PatternMatcher` (or anonymous-namespace struct) with documented `match`, `match_factors`, `assign_wildcards`, `backtrack` methods. `match_pattern` becomes a 5-line entry point: construct, call `match`, return bindings. The nested struct is invisible to file-explainer at file-scope; promotion makes the architecture legible.

**Reopen trigger:** post-promotion, blind-spot re-runs `match_pattern` → file-scope sees `PatternMatcher` as a public structural element; mechanics passes Gemma without truncation.

## #R13. Refactor: `enumerate_candidates` strategy split (194 LOC, src/system.h:2758-2951)

**From:** Cycle blind-spot 2026-05-10 (function-scope, F5). Initial T1 mechanics score: H=match/specific, **G=match/mechanism-only (truncated)** — Gemma refused even to attempt role-interpretation, paraphrased line-by-line. **Severe size signal.**

**Proposed cut points**: 7 strategies inline. Each becomes its own private method:
- `enumerate_strategy_1_direct(target, handler) → bool`
- `enumerate_strategy_2_inverse(target, handler) → bool`
- `enumerate_strategy_3_formula_forward(target, handler) → bool`
- `enumerate_strategy_4_pair_elimination(target, handler, sub_bindings) → bool`
- `enumerate_strategy_5_formula_reverse(target, handler) → bool`
- `enumerate_strategy_6_numeric(target, handler) → bool`
- `enumerate_strategy_7_two_level_elimination(target, handler, sub_bindings) → bool`

Each method returns true if the handler signaled termination. `enumerate_candidates` body becomes 7 short-circuit calls + the numeric-mode guard.

**Reopen trigger:** post-split, blind-spot re-runs `enumerate_candidates` → mechanics passes Gemma without truncation.
